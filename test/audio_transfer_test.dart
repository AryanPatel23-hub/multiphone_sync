...
void main() {
  test(
    'downloads audio to a temporary part file and reports progress',
    () async {
  ...
  test(
    'retries interrupted downloads and removes failed temporary files',
    () async {
      var attempts = 0;
      ...
      final server = await _startServer((request) async {
        ...
        if (attempts == 1) {
          await request.response.addStream(
            Stream<List<int>>.fromIterable([bytes.sublist(0, 10)]),
          );
        } else {
          await request.response.addStream(
  ...
  test('does not leave a partial file after all attempts fail', () async {
    final server = await _startServer((request) async {
      request.response.statusCode = HttpStatus.ok;
      await request.response.addStream(
        Stream<List<int>>.fromIterable([List<int>.filled(2, 1)]),
      );
      await request.response.close();
    });
    final service = LocalAudioTransferService();
