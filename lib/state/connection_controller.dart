...
class ConnectionController extends ChangeNotifier {
	...
	Future<void> connectClient({
		...
		try {
			...
			await _client.connect(
				host: host,
				port: room.port,
				connectMessage: _message(
					'CONNECT',
					{'deviceId': deviceId, 'deviceName': deviceName, 'role': 'client'},
				),
	...
	void _handleHostEvent(WebSocketMessage message) {
		...
		if (message.type != 'JOIN_ROOM' || deviceId is! String || deviceName is! String) {
			return;
		}
	...
	void _handleClientMessage(WebSocketMessage message) {
		if (message.type == 'ROOM_STATE') {
			...
			state = DeviceConnectionState.error;
			errorMessage = message.payload['message'] as String? ?? 'Connection lost.';
		}
		notifyListeners();
	}
